...
const GithubUserCard = ({username}) => {
  ...
  /* Create useState to:
    - track the status of the fetch request
    - hold the github user data
    - hold the error message
  */
 const [status, setStatus] = useState<GithubUserCardState['status']>('idle');
  const [githubUser, setGithubUser] = useState<GithubUserCardState['githubUser']>(null);
  const [error, setError] = useState<GithubUserCardState['error']>(null);
...
const RepoList = () => {
  ...
  const filteredData = repos.filter((repo) => repo.language === filterType);
  ...
  if (status === 'pending') {
    ...
    return (
      <div className="max-w-3xl p-8 mt-8 bg-white rounded-2xl min-h-[500px]">
